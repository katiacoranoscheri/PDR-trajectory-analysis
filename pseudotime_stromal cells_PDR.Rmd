...
```{r}
  ...
  data.seurat[["celltype"]] <- Idents(object = data.seurat)
...
data.monocle <- as.cell_data_set(data.seurat)
data.monocle <- cluster_cells(data.monocle)
...
plot_cells(data.monocle, color_cells_by = "celltype", label_groups_by_cluster = T, graph_label_size = 4, group_label_size = 4)
...
##test.res <- graph_test(data.monocle)
